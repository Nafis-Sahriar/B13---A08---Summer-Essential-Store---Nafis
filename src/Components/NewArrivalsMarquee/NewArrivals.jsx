  ...
  return (
    <div className="flex justify-between gap-4 bg-[#F4C83A]   mx-auto mt-10 md:sticky top-16 z-40 py-3 px-5 rounded-t-2xl">
      ...
      <Marquee pauseOnHover={true} speed={150}>
           {
            products.map((p) => <span className="px-10 flex justify-center items-center gap-2 text-[#ffffff] italic font-bold" key={p.id}> <Image src={p.image} alt={p.title} width={80} height={80} />{p.title}</span>)
           }
      </Marquee>
    </div>
  );
